...
impl AudioRecorder {
    ...
    pub fn open_with_external_source(
        ...
        let worker = std::thread::spawn(move || {
            ...
            // External sources don't use cpal's stop_flag mechanism, so create
            // a dummy flag that is never set.
            let stop_flag = Arc::new(AtomicBool::new(false));
            // Wrap Vec<f32> into AudioChunk so the shared consumer can handle them.
            let (chunk_tx, chunk_rx) = mpsc::channel::<AudioChunk>();
            std::thread::spawn(move || {
                while let Ok(samples) = sample_rx.recv() {
                    if chunk_tx.send(AudioChunk::Samples(samples)).is_err() {
                        break;
                    }
                }
            });
            run_consumer(sample_rate, vad, chunk_rx, cmd_rx, level_cb, stop_flag);
        });
